...
import { authenticateWithAxios, createClient, isAcadiaSessionExpired } from "./auth";
...
export class AcadiaImpersonator {
  ...
  constructor(studentId: string, cookies: string, clientInstance: AxiosInstance = createClient()) {
...
export async function getAcadiaImpersonator(
  ...
  const { username, password } = decryptCredentials(user.encryptedCredentials, decryptionToken);
  if (!cookies || isAcadiaSessionExpired(session.lastAcadiaAuth)) {
